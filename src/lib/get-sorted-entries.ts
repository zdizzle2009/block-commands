...
export async function getSortedEntries() {
	...
	const entries = [
		...facts.map((fact) => (Object.assign(fact, {
	category: 'facts',
	href: `/facts/${fact.slug}`
}))),
		...tutorials.map((tutorial) => (Object.assign(tutorial, {
	category: 'tutorials',
	href: `/tutorials/${tutorial.slug}`
}))),
		...videos.map((video) => (Object.assign(video, {
	category: 'videos',
...
export function sortEntries<Entry extends BaseEntry>(entries: Array<Entry>) {
	return entries.sort((a, b) => new Date(b.entry.pubDate).valueOf() - new Date(a.entry.pubDate).valueOf());
}
